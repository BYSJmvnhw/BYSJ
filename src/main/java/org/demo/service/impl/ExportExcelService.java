...
//    }
    ...
    public JSONObject saveFromExcelToStudent(HttpServletRequest request,HwUser createUser) {
        JSONObject jsonresult = new JSONObject();jsonresult.clear();
        if(createUser==null){jsonresult.put("isSuccess",false);return jsonresult;}
        String fileURL = saveExcelFile(request);
        if(fileURL == null || fileURL.isEmpty()) {
            jsonresult.put("isSuccess",false);return jsonresult;
        }
        try {
            ...
            jsonresult.put("isSuccess",true);return jsonresult;
        }catch(Exception e) {
            e.printStackTrace();
            jsonresult.put("isSuccess",false);return jsonresult;
        }
